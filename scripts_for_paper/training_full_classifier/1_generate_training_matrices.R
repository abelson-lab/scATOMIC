...
#for training we use class balanced matrices
library(dplyr)
library(data.table)
library(parallel)
library(dplyr)
...
training_seurat <- CreateSeuratObject(count_matrix)
nFeatureRNA <- training_seurat@meta.data$nFeature_RNA
names(nFeatureRNA) <- row.names(training_seurat@meta.data)
rm(training_seurat)
count_matrix <- count_matrix[, intersect(names(which(nFeatureRNA > 500 & nFeatureRNA < 6000)), colnames(count_matrix))]
combined_metadata <- combined_metadata[colnames(combined_metadata), ]
#randomize data
set.seed(123)
randomized_index <- sample(ncol(count_matrix))
training <- count_matrix[,randomized_index]
training_metadata <- combined_metadata[randomized_index,]
saveRDS(training, paste0("/unbalanced_set_full_classifier/unbalanced_training_count_mat_", layer_name,"_fold_", j, ".RDS"))
saveRDS(training_metadata, paste0("/unbalanced_set_full_classifier/unbalanced_training_metadata_", layer_name,"_fold_", j, ".RDS"))
#sample each class
...
saveRDS(training, paste0("/balanced_set_full_classifier/balanced_training_count_mat_", layer_name,".RDS"))
saveRDS(training_metadata, paste0("/balanced_set_full_classifier/balanced_training_metadata_", layer_name,".RDS"))
