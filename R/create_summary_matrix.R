#' create_summary_matrix_pan_can_v2
#'
...
create_summary_matrix <- function(raw_counts, prediction_list, use_CNVs = FALSE, modify_results = TRUE, mc.cores = (parallel::detectCores()-1),
  ...
  if(confidence_cutoff == T){
    ...
    for (i in 1:nrow(summary_master)){
      if(summary_master[i, "layer_1"] %in% c("B cell","CD4+ T cell", "CD8+ T cell", "HSPC", "macrophage_or_dendritic_cell",
    ...
    for (i in 1:nrow(summary_master)){
      if(summary_master[i, "layer_2"] %in% c("CD4+ T cell", "CD8+ T cell", "Natural killer cell", "T_or_NK_lymphocyte")){
      ...
      } else if(summary_master[i, "layer_2"] %in% c("B Cell", "B_cell_score")){
        ...
        layer_3[i] <- as.character(prediction_list[["layer_3_non_stromal"]][summary_master$cell_names[i],"predicted_tissue_with_cutoff"])
                ...
                c("Stromal Cell","Fibroblasts", "Myofibroblasts","Smooth Muscle Cells",
                  "fibroblasts_score")){
    ...
    for (i in 1:nrow(summary_master)){
      if(summary_master[i, "layer_3"]  %in% c("CD4+ T cell","CD8+ T cell" ,"CD4 or CD8 T cell")){
    ...
    if(modify_results == TRUE){
      ...
      if(use_CNVs == TRUE){
        ...
        seurat_object <- Seurat::RunUMAP(seurat_object, dims = 1:10, verbose = F)
    ...
    summary_master <- data.frame(row.names(prediction_list[["layer_1"]]),stringsAsFactors = F)
    colnames(summary_master) <- c("cell_names")
    layer_1 <- prediction_list[["layer_1"]]$predicted_class
    ...
    for (i in 1:nrow(summary_master)){
      if(summary_master[i, "layer_1"] %in% c("B cell","CD4+ T cell", "CD8+ T cell", "HSPC", "macrophage_or_dendritic_cell",
                                             "Natural killer cell","Blood_Cell")){
        layer_2[i] <- as.character(prediction_list[["layer_2_blood"]][summary_master$cell_names[i],"predicted_class"])
                ...
                c( "Bile Duct Cancer","Bladder Cancer",  "Bone Cancer",     "Brain Cancer",    "Breast Cancer",
                   ...
                   "Prostate Cancer", "Skin Cancer",  "Endothelial Cells", "Fibroblasts", "Myofibroblasts",  "Smooth Muscle Cells",
                   "Sarcoma", "Tissue_Cell_Normal_or_Cancer")){
        layer_2[i] <- as.character(prediction_list[["layer_2_non_blood"]][summary_master$cell_names[i],"predicted_class"])
    ...
    for (i in 1:nrow(summary_master)){
      if(summary_master[i, "layer_2"] %in% c("CD4+ T cell", "CD8+ T cell", "Natural killer cell", "T_or_NK_lymphocyte")){
        layer_3[i] <- as.character(prediction_list[["layer_3_TNK"]][summary_master$cell_names[i],"predicted_class"])


      } else if(summary_master[i, "layer_2"] %in% c("macrophage_or_dendritic_cell", "macrophage_DC_score")){
        layer_3[i] <- as.character(prediction_list[["layer_3_myeloid"]][summary_master$cell_names[i],"predicted_class"])

      } else if(summary_master[i, "layer_2"] %in% c("B Cell", "B_cell_score")){
        layer_3[i] <- as.character(prediction_list[["layer_3_BCell"]][summary_master$cell_names[i],"predicted_class"])
        ...
        "Prostate Cancer", "Skin Cancer",
        "Sarcoma", "Non Stromal Cell")){
        layer_3[i] <- as.character(prediction_list[["layer_3_non_stromal"]][summary_master$cell_names[i],"predicted_class"])

      } else if(summary_master[i, "layer_2"] %in%
                c("Stromal Cell","Fibroblasts", "Myofibroblasts","Smooth Muscle Cells",
                  "fibroblasts_score")){
        layer_3[i] <- as.character(prediction_list[["layer_3_stromal"]][summary_master$cell_names[i],"predicted_class"])
    ...
    for (i in 1:nrow(summary_master)){
      if(summary_master[i, "layer_3"]  %in% c("CD4+ T cell","CD8+ T cell" ,"CD4 or CD8 T cell")){
        layer_4[i] <- as.character(prediction_list[["layer_4_CD4_CD8"]][summary_master$cell_names[i],"predicted_class"])


      } else if (summary_master[i, "layer_3"] %in% c("Natural killer cell", "NK or CD8 T cell")) {
        layer_4[i] <- as.character(prediction_list[["layer_4_CD8_NK"]][summary_master$cell_names[i],"predicted_class"])

      }else if (summary_master[i, "layer_3"] == "Dendritic Cell") {
        layer_4[i] <- as.character(prediction_list[["layer_4_dendritic"]][summary_master$cell_names[i],"predicted_class"])

      }else if (summary_master[i, "layer_3"] %in%c(
        "Breast Cancer","Endometrial/Uterine Cancer", "Lung Cancer", "Ovarian Cancer",
        "Prostate Cancer", "Kidney Cancer",  "Non GI Epithelial Cell")) {
        layer_4[i] <- as.character(prediction_list[["layer_4_non_GI"]][summary_master$cell_names[i],"predicted_class"])

      } else if (summary_master[i, "layer_3"] %in% c("Bile Duct Cancer","Bladder Cancer",
                                                     "Colon/Colorectal Cancer","Esophageal Cancer",
                                                     "Gallbladder Cancer", "Gastric Cancer", "Liver Cancer",
                                                     "Pancreatic Cancer","GI Epithelial Cell")) {
        layer_4[i] <- as.character(prediction_list[["layer_4_GI"]][summary_master$cell_names[i],"predicted_class"])

      } else if (summary_master[i, "layer_3"] %in% c("Bone Cancer","Brain Cancer",
                                                     "Neuroblastoma", "Skin Cancer",
                                                     "Sarcoma", "Soft Tissue or Neuro Cancer Cell")) {
        layer_4[i] <- as.character(prediction_list[["layer_4_soft_tissue_neuro"]][summary_master$cell_names[i],"predicted_class"])
    ...
    for (i in 1:nrow(summary_master)){
      if(summary_master[i, "layer_4"] %in%c(
        "Endometrial/Uterine Cancer", "Ovarian Cancer", "Kidney Cancer", "Ovarian/Endometrial/Kidney Cell")){
        layer_5[i] <- as.character(prediction_list[["layer_5_ov_endo_kid"]][summary_master$cell_names[i],"predicted_class"])
        ...
        "Breast Cancer","Lung Cancer",
        "Prostate Cancer",  "Breast/Lung/Prostate Cell")){
        layer_5[i] <- as.character(prediction_list[["layer_5_breast_lung_prostate"]][summary_master$cell_names[i],"predicted_class"])

      } else if(summary_master[i, "layer_4"] %in% c("Bile Duct Cancer","Bladder Cancer",
                                                    "Gallbladder Cancer",  "Liver Cancer",
                                                    "Pancreatic Cancer", "Billiary Cell")){
        layer_5[i] <- as.character(prediction_list[["layer_5_biliary"]][summary_master$cell_names[i],"predicted_class"])

      } else if(summary_master[i, "layer_4"] %in% c("Colon/Colorectal Cancer","Esophageal Cancer",
                                                    "Gastric Cancer","Colorectal/Esophageal/Gastric Cell")){
        layer_5[i] <- as.character(prediction_list[["layer_5_digestive"]][summary_master$cell_names[i],"predicted_class"])

      } else if(summary_master[i, "layer_4"] %in% c("Bone Cancer","Brain Cancer",
                                                    "Neuroblastoma",
                                                    "Sarcoma", "Soft Tissue or Neuro Cancer Cell", "Soft Tissue or Neuro Cancer Cell")){
        layer_5[i] <- as.character(prediction_list[["layer_5_soft_tissue_neuro"]][summary_master$cell_names[i],"predicted_class"])
    ...
    for (i in 1:nrow(summary_master)){
      if(summary_master[i, "layer_5"] %in% c("Brain Cancer",
                                             "Neuroblastoma", "Brain/Neuroblastoma Cancer Cell")){
        layer_6[i] <- as.character(prediction_list[["layer_6_brain_nbm"]][summary_master$cell_names[i],"predicted_class"])

      } else if(summary_master[i, "layer_5"] %in% c("Bone Cancer",
                                                    "Sarcoma", "Soft Tissue Cancer Cell")){
        layer_6[i] <- as.character(prediction_list[["layer_6_soft_tissue"]][summary_master$cell_names[i],"predicted_class"])
